...
<div class="row justify-content-center">
    <div class="col-md-8">
        <h1 class="fw-bold py-3 mb-4 text-center">تعديل المقال</h1>
        <div class="card">
            <div class="card-body">
                <form action="{{ route('dashboard.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="mt-3">
                            <img id="preview" src="{{ $blog->image ? asset('storage/' . $blog->image) : '#' }}" alt="معاينة الصورة" class="{{ $blog->image ? '' : 'd-none' }} img-thumbnail" width="100">
                        </div>
                        @error('image')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control @error('is_published') is-invalid @enderror" name="is_published" required>
                            <option value="1" {{ old('is_published', $blog->is_published) == '1' ? 'selected' : '' }}>منشور</option>
                            <option value="0" {{ old('is_published', $blog->is_published) == '0' ? 'selected' : '' }}>غير منشور</option>
                        </select>
                        @error('is_published')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
